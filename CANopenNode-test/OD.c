...
OD_ATTR_RAM OD_RAM_t OD_RAM = {
    ...
    /* Read-only fixed string used to exercise segmented uploads.
     * Length must be > 4 so the SDO server picks segmented mode.
     * Padded with NULs up to OD_LEN_X2000_TEST_STRING. */
    .x2000_testString = "canopen-sdo segmented upload test 0123456789 ABCDEFGHIJ",
    /* Read/write buffer used to exercise segmented downloads + round-trip. */
    .x2001_testBuffer = {0}
};
...
typedef struct {
    ...
    OD_obj_var_t o_2000_testString;
    OD_obj_var_t o_2001_testBuffer;
} ODObjs_t;
...
static CO_PROGMEM ODObjs_t ODObjs = {
    ...
    .o_2000_testString = {
        .dataOrig = &OD_RAM.x2000_testString[0],
        .attribute = ODA_SDO_R | ODA_STR,
        .dataLength = OD_LEN_X2000_TEST_STRING
    },
    .o_2001_testBuffer = {
        .dataOrig = &OD_RAM.x2001_testBuffer[0],
        /* No ODA_STR: this is binary, not a NUL-terminated string. With
         * ODA_STR the SDO server uses strnlen() on read, so a buffer that
         * starts with 0x00 is reported as length 1 — not what we want for
         * a fixed-size 256 B blob. Without ODA_STR, reads always return
         * exactly OD_LEN_X2001_TEST_BUFFER and writes must match the size. */
        .attribute = ODA_SDO_RW,
        .dataLength = OD_LEN_X2001_TEST_BUFFER
    }
};
...
static OD_ATTR_OD OD_entry_t ODList[] = {
    ...
    {0x2000, 0x01, ODT_VAR, &ODObjs.o_2000_testString, NULL},
    {0x2001, 0x01, ODT_VAR, &ODObjs.o_2001_testBuffer, NULL},
